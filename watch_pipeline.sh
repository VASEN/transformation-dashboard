...
# что и сам пайплайн (extract_data.REDMINE_REQUIRED).
#
# Сама проверка — в check_source.py: тот же вопрос задаёт Telegram-бот, принимая
# файл из чата, и двух копий проверки быть не должно (вынесено 10.09.2026).
# Код возврата различает два разных исхода: 4 — файл действительно чужой,
# всё прочее ненулевое — проверка не выполнилась (сломанная среда, недоступный
# файл). Во втором случае помечать выгрузку обработанной нельзя: диагноз
# неизвестен, и после починки среды она должна подхватиться сама.
#
# Вердикт «чужой» — 4, а не 2: код 2 отдаёт сам интерпретатор, когда не может
# открыть файл скрипта. Отсутствие check_source.py тогда выглядело бы как
# «в выгрузке нет колонок Redmine», и настоящая выгрузка уходила бы в failed
# безвозвратно (найдено независимым ревью 10.09.2026). Поэтому же наличие
# скрипта проверяется до вызова — иначе диагноз ставит не он.
if [ ! -f "$PROJECT_DIR/check_source.py" ]; then
  check_out="проверка не выполнилась: нет $PROJECT_DIR/check_source.py"
  check_rc=3
else
  check_out=$(python3 "$PROJECT_DIR/check_source.py" "$latest" 2>&1)
  check_rc=$?
fi
